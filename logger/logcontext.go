...
func (l LogContext) submit(sc *ServiceContext, lc *LogContext, ld LogDetail) {
	...
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(sc.Environment)], sc.Environment)
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(systemNameToken)], systemNameToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(sc.SystemName)], sc.SystemName)
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(serviceNameToken)], serviceNameToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(sc.ServiceName)], sc.ServiceName)
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(serviceInstanceIDToken)], serviceInstanceIDToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(sc.ServiceInstanceID)], sc.ServiceInstanceID)
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(siteToken)], siteToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(lc.Site)], lc.Site)
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(operationToken)], operationToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(lc.Operation)], lc.Operation)
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(levelToken)], levelToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(string(ld.Level))], string(ld.Level))
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(severityToken)], severityToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(string(ld.Severity))], string(ld.Severity))
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(messageToken)], messageToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(ld.Message)], ld.Message)
	offset += copy(lc.buffer[offset:offset+len(fieldCloseToken)], fieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(detailsToken)], detailsToken)
	offset += copy(lc.buffer[offset:offset+len(fieldOpenToken)], fieldOpenToken)
	offset += copy(lc.buffer[offset:offset+len(ld.Details)], ld.Details)
	offset += copy(lc.buffer[offset:offset+len(finalFieldCloseToken)], finalFieldCloseToken)
	offset += copy(lc.buffer[offset:offset+len(braceCloseToken)], braceCloseToken)
